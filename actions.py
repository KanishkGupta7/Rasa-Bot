...
class DaysOffMail(Action):
  ...
  def run(self, dispatcher, tracker, domain):
      ...
      msg = EmailMessage()
      msg['Subject'] = 'Enquiry details about property '
      msg['From'] = 'kanishkgupta1998@gmail.com'  #sender's email address
      msg['To'] = to_email  #receiver's email address
      msg.set_content('Hi {},\n\n We have succesfully taken your details ,our agent will get back to you soon  '.format(name)) 
      with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
        smtp.login('kanishkgupta1998@gmail.com', 'Iamkanishkgupta123')	#replace with your email and password
        smtp.send_message(msg)

      if intentions == 'buy':
        if location:
            print("===================*********** BUY **************===============")
            stri='http:/13.233.151.164:8080/listings/search?keywords=&city=%s'%(location.split(' ')[1])
            ...
            stri='http://13.233.151.164:8080/listings/'
        ...
        if location:
            ...
            if len(location.split())>1:
              ...
              loc=location
            payload = {"realtor":2,"sqft":2000,"city":loc,"address":loc,"zipcode":pincode,"title":"Buy","state":loc,"price":budget.split(' ')[0],"bedrooms":2,"bathrooms":2,"lot_size":2000}
           # payload=json.dumps(payload)
            print("PAyload",payload)
            response = requests.post(url="http://13.233.151.164:8080/listings/api/listing",json=payload) 
            print("RESPONSE",response.status_code)
            if response.status_code==201:
                dispatcher.utter_message("Hi {} we have successfully listed your property {} :)".format(name,response.text))
                return [UserUtteranceReverted()] 
            else:
                dispatcher.utter_message("Sorry we cant list your property right now please come back later ".format(name,response.text))
                return [UserUtteranceReverted()] 
